...
local function menu_cityhall(self)
  ...
  vRP.EXT.GUI:registerMenuBuilder("cityhall", function(menu)
    ...
    menu:addOption(lang.identity.cityhall.new_identity.title(), m_new_identity,
      lang.identity.cityhall.new_identity.description({ self.cfg.new_identity_cost }))
  end)
end
...
local function menu_identity(self)
  vRP.EXT.GUI:registerMenuBuilder("identity", function(menu)
    ...
    if identity then
      menu:addOption(lang.identity.citizenship.title(), nil,
        lang.identity.citizenship.info({ htmlEntities.encode(identity.name), htmlEntities.encode(identity.firstname),
          identity.age, identity.registration, identity.phone }))
    end
  end)
end
...
function Identity:__construct()
  ...
  async(function()
    ...
    vRP:prepare("vRP/init_character_identity",
      "INSERT IGNORE INTO vrp_character_identities(character_id,registration,phone,firstname,name,age) VALUES(@character_id,@registration,@phone,@firstname,@name,@age)")
    vRP:prepare("vRP/update_character_identity",
      "UPDATE vrp_character_identities SET firstname = @firstname, name = @name, age = @age, registration = @registration, phone = @phone WHERE character_id = @character_id")
    vRP:prepare("vRP/get_characterbyreg",
      "SELECT character_id FROM vrp_character_identities WHERE registration = @registration")
