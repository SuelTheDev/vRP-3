...
Menu.prototype.open = function (data) {
  ...
  this.div_options.style.height =
    this.div.offsetHeight - this.div_options.offsetTop + "px";
...
Menu.prototype.updateOption = function (i, title, description) {
  if (i >= 0 && i < this.options.length) {
    ...
    if (description) {
      if (option.length > 1) option[1] = description;
      else option.push(description);

      if (this.selected == i) {
        this.div_desc.innerHTML = option[1];

        this.div_desc.style.display = "block";
        this.div_desc.style.left =
          this.div.offsetLeft + this.div.offsetWidth + "px";
...
Menu.prototype.setSelected = function (i) {
  ...
  else if (this.selected >= this.options.length) this.selected = 0;
...
Menu.prototype.valid = function (mod) {
  if (this.selected >= 0 && this.selected < this.options.length) {
    if (this.onValid && this.opened) this.onValid(this.selected, mod);
  }
};
