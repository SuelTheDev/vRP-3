...
function Proxy.addInterface(name, itable)
  ...
  Proxy.interfaces[name] = AddEventHandler(name .. ":proxy", function(member, args, identifier, rid)
...
function Proxy.removeInterface(name)
  if Proxy.interfaces[name] then
    RemoveEventHandler(Proxy.interfaces[name])
    Proxy.interfaces[name] = nil
    return true
  end
